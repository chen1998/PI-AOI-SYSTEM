...
# Logging
# =========================================================
def setup_logger(log_dir: str = "logs", name: str = "build_api_summary_from_rtms_aoi300_job") -> logging.Logger:
...
# Target table
# =========================================================
def ensure_api_summary_table(db: MySQLDB, table_name: str):
    ddl = f"""
...
def upsert_api_summary_df(db: MySQLDB, table_name: str, df: pd.DataFrame):
    if df is None or df.empty:
        logger.info(f"[upsert_api_summary_df] {table_name}: no rows")
        return

    ensure_api_summary_table(db, table_name)
    ...
    if not rows:
        logger.info(f"[upsert_api_summary_df] {table_name}: no rows after conversion")
        return
    ...
    logger.info(f"[upsert_api_summary_df] {table_name}: upserted {len(rows)} rows")
...
def api_table_name(yyyymm: str) -> str:
    return f"aoi300_{yyyymm}_api_summary_table"
...
def normalize_size(v) -> str:
    ...
    說明：
    - 真實 defect：S/M/L/O
    - macro 原點影像：defect_size = OK，size_class = 0
    - size_class = 0 視為 OK
    - 無法判斷時回傳空字串，避免被誤算成 S
    """
...
def aggregate_raw_summary(raw_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 新版 raw：defect_size 已經是 S/M/L/O
    d["size_norm"] = d["defect_size"].map(normalize_size)

    # 若 defect_size 空，才 fallback 用 size_class（面積值）
...
def build_api_summary(raw_summary_df: pd.DataFrame, glass_df: pd.DataFrame) -> pd.DataFrame:
    if raw_summary_df is None or raw_summary_df.empty:
        return pd.DataFrame()

    d = raw_summary_df.copy()
    ...
    if glass_df is None or glass_df.empty:
        ...
        d["pi_time"] = None
        d["pi_type"] = "Null"
        d["run_day"] = pd.to_datetime(d["test_time"], errors="coerce").dt.normalize()
        return d
    ...
    merged["line_id"] = merged["line_id"].fillna("Null")
    merged["aoi"] = merged["aoi"].fillna("aoi300")
    merged["pi_type"] = merged["pi_type"].fillna("Null")
    merged["run_day"] = merged["run_day"].fillna(pd.to_datetime(merged["test_time"], errors="coerce").dt.normalize())
...
def one_run(
    ...
    for ym in months:
        ...
        ot = api_table_name(ym)

        if not raw_db.table_exists(rt):
            logger.info(f"[one_run] skip raw table not exists: {rt}")
            continue

        raw_df = load_raw_rows(raw_db, rt, start_dt, end_dt)
        if raw_df.empty:
            logger.info(f"[one_run] empty raw rows: {rt}")
            continue

        raw_summary_df = aggregate_raw_summary(raw_df)
        if raw_summary_df.empty:
            logger.info(f"[one_run] empty raw summary: {rt}")
            continue

        glass_df = pd.DataFrame()
        if raw_db.table_exists(gt):
            glass_df = load_glass_rows(raw_db, gt, start_dt, end_dt)

        api_df = build_api_summary(raw_summary_df, glass_df)
        if api_df.empty:
            logger.info(f"[one_run] empty api df: {ym}")
            continue

        upsert_api_summary_df(out_db, ot, api_df)
        logger.info(f"[one_run] upsert {ot}: {len(api_df)} rows")
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Build API summary from RTMS AOI300")
...
"""
# 單次執行：最近 3 小時
python build_api_summary_from_rtms_aoi300_job.py --once

# 單次執行：指定區間
python build_api_summary_from_rtms_aoi300_job.py --once --start-dt "2026-05-01 07:30:00" --end-dt "2026-05-07 17:30:00"

# 單次執行：指定單日
python build_api_summary_from_rtms_aoi300_job.py --once --date 2026-04-14

# 常駐，每 10 分鐘跑一次
python build_api_summary_from_rtms_aoi300_job.py
"""