    ...
    def append_or_create_dedup(
        ...
        *,
        ...
        with self.engine.begin() as conn:
            ...
            base = f"__stg_{table_name}"
            max_base_len = 64 - 1 - len(suffix)
            base = base[:max_base_len]

            stg_name = f"{base}_{suffix}"
            stg_qual = self._qual_table(stg_name)

            try:
                # 不用 pandas 自動推斷 staging schema，避免 LONGTEXT 被推成 TEXT
                conn.execute(text(f"DROP TABLE IF EXISTS {stg_qual}"))
                conn.execute(text(f"CREATE TABLE {stg_qual} LIKE {tbl_qual}"))

                # 只保留這次會寫入的欄位；LIKE 會把正式表完整 schema 複製過來，
                # 所以 point_detail 會維持 LONGTEXT，不會被 pandas 建成 TEXT。
                df_use.to_sql(
                    name=stg_name,
                    con=self.engine,
                    schema=self.db,
