...
(function () {
  ...
  // -------------------------
  // 管理 inspection density 底下 3 個 section
  // -------------------------
  const INSP_SECTION_IDS = [
    "aoi-inspection-density-root",        // 主畫面（Hourly）
    "aoi-inspection-density-spec-table",  // Spec Table
    "aoi-inspection-density-chart-root",   // Trend Chart
  ...
  AOI.fetchInspectionData = async function (opts = {}) {
    ...
    const rows = src.map((r) => {
      ...
      const totalDensity = toNum(r.maingroup_density);
  ...
  AOI.fetchTrend = async function (extra) {
    ...
    console.debug("[AOI_INSPECTION.fetchTrend] POST /trend payload =", JSON.parse(JSON.stringify(body)));
    ...
    try {
      ...
      console.debug("[AOI_INSPECTION.fetchTrend] response points =", {
        summary: summaryN, month: monthN, week: weekN, day: dayN
      });
    ...
    if (type === "table") {
      ...
      if (tabKey === "EditSummary") {
        ...
        document.dispatchEvent(new CustomEvent("aoi_inspection:subtab-table", {
          detail: { tabKey, config: defs, data: resp?.DictData || {} }
        }));
        return;
      }
    ...
    if (type === "Chart") {
      ...
      if (!AOI.state.chartInited) {
        ...
        document.dispatchEvent(new CustomEvent("aoi_inspection:subtab-chart", {
          detail: { tabKey, config: defs, trend: null, filter: {}, restoreOnly: false }
        }));
      } else {
        document.dispatchEvent(new CustomEvent("aoi_inspection:subtab-chart", {
          detail: { tabKey, config: defs, trend: null, filter: {}, restoreOnly: true }
        }));
      }
      return;
    }
