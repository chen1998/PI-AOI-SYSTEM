...
// static/js/aoi_inspection_density/function/defect_map.js
// 單一 canvas 顯示所有 defect；以尺寸 S/M/L/O 著色；右側圖例含尺寸/Glass 篩選；點擊開圖；hover 顯示資訊
// 版本重點：
// 1) 符合 aoi_inspection_density 新資料結構
// 2) 支援 system_tab.js lazy load（不能只靠 DOMContentLoaded）
// 3) 保留尺寸篩選 / glass 勾選顯示功能
// 4) 避免重複綁定事件
...
(function () {
  ...
  const CANVAS_ID = "aoi-inspection-density-mini-map";
  const LEGEND_ID = "aoi-inspection-density-map-legend";
  const TIPS_ID = "aoi-inspection-density-map-tooltip";

  // 新資料結構下先顯示實際穩定可用欄位
  const DEFECT_INFO_KEYS = ["glass_id", "x", "y", "defect_size"];
  ...
  function ensureLayout() {
    const host = document.getElementById("aoi-inspection-density-map-wrap");
    const vp = host?.querySelector(".map-viewport");
    const cvs = document.getElementById(CANVAS_ID);
    if (!host || !vp || !cvs) return null;
    ...
    return { host, vp, cvs, legend, tips };
  }
  ...
  function normalizeBucket(rawVal) {
    const raw = String(rawVal ?? "").trim().toUpperCase();
    if (!raw) return null;

    if (["S", "M", "L", "O"].includes(raw)) return raw;
  ...
  function normalizeResponse(resp) {
    ...
    if (!Array.isArray(src) && typeof src === "object") {
      Object.entries(src).forEach(([gid, arr]) => {
        ...
        arr.forEach((it) => {
          const xNum = Number(it?.x);
          const yNum = Number(it?.y);
          if (!Number.isFinite(xNum) || !Number.isFinite(yNum)) return;

          const bucket = normalizeBucket(it?.size);
    ...
    (groups || [])
      ...
      .forEach((g) => {
        ...
        Object.assign(row.style, {
          ...
          borderRadius: "4px",
          border: "1px solid rgba(255,255,255,0.08)",
          background: state.glassFilter.has(g.gid)
        ...
        cb.addEventListener("change", () => {
          ...
          row.style.background = state.glassFilter.has(g.gid)
            ? "rgba(255,255,255,0.04)"
            : "transparent";
  ...
  MOD.Map.fetchAndRender = async function (filterRows) {
    ...
    try {
      ...
      if (API?.postDefectMap) {
        resp = await API.postDefectMap(rows);
      } else if (API?.post) {
        const base = window.API_BASE_DENSITY || window.API_BASE || "";
        const url = `${base}/aoi_inspection_density/defect_map`;
        resp = await API.post(url, { rows });
      }
  ...
  MOD.Map.ensureInit = function () {
    const ok = initMapOnce();
    if (!ok) scheduleInitRetry();
  };
