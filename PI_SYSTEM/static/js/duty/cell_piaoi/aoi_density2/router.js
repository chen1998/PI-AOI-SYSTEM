...
(function () {
  ...
  ns.ensureInit = async function () {
    try {
      ...
      if (!dates) {
        ...
        await fetchData(dates, {}, 'init-with-ui-dates');
  ...
  function setUIDates([b, e]) {
    const beginEl = document.querySelector("#aoi-density-start");
    const endEl   = document.querySelector("#aoi-density-end");
  ...
  async function redraw() {
    ...
    if (!ns.justFetchedUsingDefault) {
      if (dates && !isWithinCachedSpan(dates) && ns.userAppliedDate) {
        ...
        await fetchData(dates, readFilters(), 'redraw-out-of-cache');
      }
    } else {
      ns.justFetchedUsingDefault = false;
    }
  ...
  function bindUI() {
    ...
    const right = document.getElementById("aoi-density-right");
    if (right) {
      right.addEventListener("change", (ev)=>{
        const t = ev.target;
        if (t && t.tagName === "SELECT" && /^f-/.test(t.id)) {
          if (t.id === "f-same-point-offset") return;
          redraw();
        }
      });
    ...
    document.getElementById("aoi-density-apply")?.addEventListener("click", async ()=>{
      const dates = readDates();
      console.log('dates',dates);
      if (!dates) return;
      ns.userAppliedDate = true;
      if (!isWithinCachedSpan(dates)) {
        await fetchData(dates, readFilters(), 'apply-out-of-cache');
      }
      await redraw();
    });

    document.getElementById("aoi-density-clear")?.addEventListener("click", async ()=>{
      const b = document.getElementById("aoi-density-start");
      const e = document.getElementById("aoi-density-end");
      if (b) b.value = ""; if (e) e.value = "";
      ns.userAppliedDate = false;
      window.AOI_DENSITY.state.skipReapplySubTabOnce = true;
  ...
  function render(rows){
    const root = document.getElementById("aoi-density-root");
    if (!root) return;
