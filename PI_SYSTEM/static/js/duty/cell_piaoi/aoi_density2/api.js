...
(function () {
  //if (window.AOI_DENSITY_API) return;
  ...
  // ===== 對外：AOI Density Trend =====
  async function postTrend(body) {
    const url = `${window.API_BASE}/aoi_density/trend`;
  ...
  async function resetSummaryFilter(params) {
    ...
    return get(`${window.API_BASE}/aoi_density/reset_summary_filter`, params);
  }
  ...
  async function postDefectMap(rows) {
    ...
    const url = `${window.API_BASE}/aoi_density/defect_map`;
    const body = { rows };
  ...
  async function getRecipeSamePoint(params) {
    const url = `${window.API_BASE}/aoi_density/recipe_same_point`;
  
    // SamePoint 不走防重複鎖，避免初始切 Total tab 時被 _withLock 回傳 null。
    return _rawGet(url, params);
  }
  ...
  //  新增：default_spec_table 編輯 / 新增
  async function FrontSpecEditor(payload) {
    //console.log('density payload', payload);
    const url = `${window.API_BASE}/common/spec_editor`;
    return post(url, payload);
  }
  ...
  // ===== 對外：AOI Density Mail Alert =====
  async function postMailAlert(body) {
    const url = `${window.API_BASE}/common/mail_alert`;
    return post(url, body);
  }

  window.AOI_DENSITY_API = {
    get,
    post,
    resetSummaryFilter,
    postDefectMap,
