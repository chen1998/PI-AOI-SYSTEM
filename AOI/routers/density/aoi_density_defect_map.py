...
def group_defects_by_glass(rows, uni_glasses) :
    out: Dict[str, Dict] = {}
    img_flds = []
    for r in rows or []:
    ...
    for gid in uni_glasses:
        if gid not in out.keys():
            g = out.setdefault(gid, {"defect_map": [], "S": 0, "M": 0, "L": 0, "O": 0, "total": 0})
    ...
    try:
        with self.engine.begin() as conn:
            ...
            if not rows:
                ...
                # 2) 依序 AND 疊加
                order = ["pi_hour", "line_id",  "model", "glass_type", "recipe_id", "ai_code_1"]
...
def get_group_glass_names(dbhandler, tbn, key_dict):
    key_dict ={k:key_dict.get(k) for k in  ['pi_hour', 'aoi', 'line_id', 'model','glass_type', 'recipe_id']}
    #print('key_dict',key_dict)
    rows = dbhandler.get_rows(tbn, key_dict)
    #print('rows', rows)
    
    glasses = [r.get('glass') for r in rows]
    total_glass_names = []
    for g in glasses:
        total_glass_names = total_glass_names + g.split(",")
    uni_glasses = [v for v in set(total_glass_names)]
    #print(len(uni_glasses ), uni_glasses )
...
async def defect_map(payload: DefectMapIn):
    ...
    for filters in payload.rows:
        ...
        logging.info(f"[Density defect_map] tb={tbname} key_dict={key_dict}")
        uni_glasses = get_group_glass_names(dbhandler, f"pidenisty_pihour_20{yymm}", key_dict)
        ...
        # 分組輸出
        #print(rows)
        filters['defect_group'] = group_defects_by_glass(rows, uni_glasses)
        out_rows.append(filters)
