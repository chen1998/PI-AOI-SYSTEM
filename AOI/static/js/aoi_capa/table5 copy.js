...
(function () {
  ...
  function ensureDetailPanel() {
    ...
    saveBtn?.addEventListener("click", async () => {
      ...
      // 同步回 AOI.state.rows / baseDayRows
      try {
        const rows = AOI.state?.rows || [];
        rows.forEach((r) => {
          if (
            String(r.aoi || "") === aoi &&
            String(r.run_day || "") === run_day &&
            String(r.pi_type || "") === pi_type
          ) {
            r.comment = newVal;
          }
        });
        const base = AOI.state?.baseDayRows || [];
        base.forEach((r) => {
          if (
            String(r.aoi || "") === aoi &&
            String(r.run_day || "") === run_day &&
        ...
        console.warn("[AOI_CAPA] update state comment failed:", e);
      }
  ...
  AOI.Table.showHourly = async function (meta) {
    ...
    // ★ 需求：不管點哪個顏色柱狀，一律顯示該日「所有 pi_type + ALL」
    // 所以這裡不要用 meta.pi_type，直接固定用 ALL
    const pt = "ALL";
