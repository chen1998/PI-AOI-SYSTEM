// static/js/aoi_inspection/multidd.js
// 多選下拉元件（checkbox + 搜尋 + 全選/清空 單一切換鈕）— inspection 版
(function(){
  ...
  class MultiDD {
    ...
    render() {
      ...
      document.addEventListener('click', (ev)=>{
        if (!wrap.contains(ev.target)) { list.style.display = 'none'; wrap.classList.remove('open'); }
      });
    ...
    updateOptions(options=[]) {
      this.options = options.slice().sort();
      this.selected.forEach(v => { if (!this.options.includes(v)) this.selected.delete(v); });
    ...
    _syncSelect(select) {
      Array.from(select.options).forEach(o => { o.selected = this.selected.has(o.value); });
    }
    _updateBtnText(btn) {
