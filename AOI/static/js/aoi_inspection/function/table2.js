...
(function () {
  ...
  function upsertEditSummaryFromRow(row) {
    ...
    // 允許後端是 dict（index→row），這裡統一轉成 array 來操作
    ...
    // 若原本 history 裡沒有這列，但現在有 comment 或 action，就新增一筆
    ...
    // 如果現在正在看 EditSummary tab，就順便重畫 table_tab
    try {
      if (AOI.currentSectionId === 'inspection-spec-table'
          && AOI.state.activeSubTab === 'EditSummary') {

        const cfg =
          AOI.state.paramDict?.SubTabsFilterDefaultDict?.EditSummary || {};

        document.dispatchEvent(new CustomEvent("aoi_inspection:subtab-table", {
          detail: {
            tabKey: 'EditSummary',
            config: cfg,
            data: hist        // 直接丟 array 給 table_tab
          }
        }));
      }
    } catch (e) {
