// static/js/aoi_inspection/service/api1.js
(function () {
  ...
  async function resetSummaryFilter(params) {
    ...
    return get(`${window.API_BASE}/aoi_inspection/api/reset_summary_filter`, params);
  }
  ...
  async function postDefectMap(rows) {
   ...
   // ===== 新增：inspection 趨勢圖 API =====
   async function getInspectionTrend(payload) {
    const url = `${window.API_BASE}/aoi_inspection/api/trend`;
    //console.log('url',url);
    const body = payload || {};
    const r = await _withLock("POST", url, body, () => _rawPost(url, body), 300);
    console.log('r',r);
  ...
  // ===== 新增：EditSummary 依日期重撈 =====
  async function fetchActionEditSummary(payload) {
    const url = `${window.API_BASE}/api/edit_summary`;
    const body = payload || {};
    // 和 other POST 一樣，用 _withLock 包一下
    return _withLock("POST", url, body, () => _rawPost(url, body), 300);
  }


   //  新增：default_spec_table 編輯 / 新增
  async function frontEditor(payload) {
    const url = `${window.API_BASE}/api/front_editor`;
  ...
  window.API = {
    ...
    resetSummaryFilter,
    postDefectMap,
    fetchActionEditSummary,
    getInspectionTrend,   // <-- 新增匯出
    frontEditor
  };
})();
