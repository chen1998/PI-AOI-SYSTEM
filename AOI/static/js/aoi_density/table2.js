...
(function () {
  ...
  function parseGlassList(val) {
    if (val == null) return [];
    if (Array.isArray(val)) return val.map(fString).map((s) => s.trim()).filter(Boolean);
  ...
  function getPerGlassSet(colDict, sampleRow){
    const out = new Set();
    if (!colDict) return out;
    Object.keys(colDict).forEach(k=>{
      if (/glass/i.test(k)) out.add(k);
    });
    if ('glass' in sampleRow) out.add('glass');
    if ('glass_defect_count' in sampleRow) out.add('glass_defect_count');
    return out;
  }
  ...
  function injectGlassCounts(requestRows, response) {
    console.log('injectGlassCount', requestRows);
    ...
    const out = (requestRows || []).map((r, idx) => {
      const src = list[idx] || {};
      const dg  = src.defect_group || {};   // { glass_id: {S,M,L,O,total}, ... }
  
      // 1) 先把原本 row 裡的 glass 清單整理成陣列
      const origGlassList = Array.isArray(r.glass)
        ? r.glass.map(s => String(s).trim()).filter(Boolean)
        : String(r.glass || "")
            .split(",")
            .map(s => s.trim())
      ...
      // 2) 從 defect_map 回傳拿到所有 glass_id
      const respGlassList = Object.keys(dg || {}).map(s => String(s).trim()).filter(Boolean);
  
      // 3) 合併：先保留原本順序，再把回傳裡多出來的 glass append 上去
      const mergedGlassSet = new Set(origGlassList);
      respGlassList.forEach(gid => {
        if (gid && !mergedGlassSet.has(gid)) {
          mergedGlassSet.add(gid);
        }
      });
      const mergedGlass = Array.from(mergedGlassSet);  // 這個會變成新的 r.glass（陣列）
  
      // 4) 組新的 glass_defect_count：只有 defect_map 有提供 payload 的才填 S/M/L/O/T
      const gdc = {};
      mergedGlass.forEach((gid) => {
        const payload = dg[gid] || {};
        console.log(payload);
        ...
        // 有實際統計才給字串，全部 0 的可以視情況要不要顯示
        gdc[gid] = `S:${S} M:${M} L:${L} O:${O} T:${T}`;
      });
  
      // 5) 回傳新的 row：
      //    - glass 改成 mergedGlass（陣列）
      //    - glass_defect_count 帶入剛剛算好的 gdc
      //    - __no_api: true 防止 showRows 再打一次 defect_map
      return {
        ...r,
        glass: mergedGlass,
        glass_defect_count: gdc,
  ...
  document.addEventListener("aoi_density:defect-map-ready", (ev) => {
    ...
    const pd   = window.AOI_DENSITY?.state?.paramDict || {};
    //console.log('req',req);
    // ★ 如果沒有 requestRows，就不要動現在的 Table（避免洗掉）
    if (!Array.isArray(req) || !req.length) {
      console.warn("[aoi_density] defect-map-ready with empty requestRows, skip Table.showRows");
      return;
    }
