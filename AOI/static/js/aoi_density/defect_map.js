// static/js/aoi_density/defect_map.js
// 單一 canvas 顯示所有 defect；以「尺寸 S/M/L/O」著色；右側圖例含 尺寸多選 + Glass/Recipe 勾選；點擊點開影像；hover 顯示資訊
(function () {
  const MOD = (window.AOI_DENSITY = window.AOI_DENSITY || {});
  MOD.Map = MOD.Map || {};

  const CANVAS_ID = "aoi_density-mini-map";
  const LEGEND_ID = "aoi_density-map-legend";
  const TIPS_ID   = "aoi_density-map-tooltip";

  // 懸浮框欄位順序（依需求）
  const DEFECT_INFO_KEYS = ["lotid", "recipe", "x", "y", "defect_size", "ai_code_1"];

  // 固定座標範圍（µm）
  const AXIS = { minX: 0, maxX: 1850000, minY: 0, maxY: 1500000 };
  ...
  function ensureLayout() {
    const host = document.getElementById("aoi_density-map-wrap");
  ...
  // ===== 狀態 =====
  const state = {
    groups: [],                               // [{gid, label, points:[{x,y,bucket,rawSize,img,chip,recipe_id,meta}]}]
    sizeFilter: new Set(["S","M","L","O"]),   // 尺寸多選
    glassFilter: new Set(),                   // 勾選可見的 gid（預設全選）
    hitCache: [],                             // [{sx,sy,r,url,point}]
  };

  // ===== 後端 → 正規化 =====
  function normalizeResponse(resp) {
    const outMap = new Map();
    const list = (resp && resp.DefectGroupDict) || [];
  ...
  function buildLegend(groups) {
    ...
    ["S","M","L","O"].forEach(k=>{
      ...
      item.addEventListener("click", ()=>{
        ...
        buildLegend(state.groups); // 讓按鈕文案立刻更新（全選/清空）
        redraw();
      });
  ...
  function redraw() {
    ...
    function toScreen(p) {
      ...
      const y = coord.padT + (p.y - coord.y0) * coord.yScale;   // ★ 不翻轉
      return [x, y];
    }
  ...
  MOD.Map.setData = function(response) {
    ...
    state.sizeFilter = new Set(["S","M","L","O"]);
    state.glassFilter = new Set((state.groups || []).map(g=>g.gid)); // 預設全選
    buildLegend(state.groups);
    redraw();
  };
  ...
  function bindCanvasEvents() {
    ...
    cvs.addEventListener("click", (ev)=>{
      const hit = findHit(ev);
      if (hit && hit.url) {
        //const url = buildImageUrl(hit.url);
