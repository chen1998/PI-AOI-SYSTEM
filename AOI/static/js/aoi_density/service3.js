...
(function () {
  ...
  // 管理 aoi_density 底下 3 個 section
  const DENSITY_SECTION_IDS = [
    "aoi_density-root",   // 原本 Density 主畫面
    "aoi_density-spec-table",  // 你新增的 Table 頁
    "aoi_density-Chart"   // 你新增的 Chart 頁
  ];

  AOI.currentSectionId = "aoi_density-root";   // 預設顯示哪一個 section
  ...
  AOI.showSection = function(sectionId){
    AOI.currentSectionId = sectionId || "aoi_density-root";
    DENSITY_SECTION_IDS.forEach(id=>{
      const el = document.getElementById(id);
      if (!el) return;
  ...
  // 給 system_tab 在切回 aoi_density 時呼叫用
  AOI.syncSectionVisibility = function(){
    AOI.showSection(AOI.currentSectionId || "aoi_density-root");
  };
  ...
  */
  ...
  AOI.applyDensitySubTabFilters = function(tabKey){
   ...
   // === 新版：根據 SubTab 的 type 決定要切到哪個 section ===
   AOI.applySubTab = function(tabKey){
    
    const pd = AOI.state.paramDict || {};
    const rows = AOI.state.ProSpecDict?.[tabKey] || {};
    const defs = pd.SubTabsFilterDefaultDict?.[tabKey] || {};
    const type = defs.type || "";   // 可能是 "", "table", "Chart"
    //console.log(tabKey, 'AOI.state.ProSpecDict',rows);
    // 先記錄 active tab key
    AOI.state.activeSubTab = tabKey;
    density_sub_activeTabKey = tabKey;

    // 先把所有 aoi_density section 關掉
    AOI.hideAllSections();

    if (!type) {
      // === 沒 type：舊 UPI/PISpot/SPS → 顯示主 Density 畫面 ===
      AOI.showSection("aoi_density-root");
      AOI.applyDensitySubTabFilters(tabKey);

      // 主畫面通常還會需要 redraw
      //（這裡你可以視情況呼叫 router.redraw 或在 change event 中 redraw）
      document.dispatchEvent(new CustomEvent("aoi_density:subtab-density", { detail: { tabKey } }));

    } else if (type === "table") {
      // === type: "table" → Spec Table 頁 ===
      AOI.showSection("aoi_density-spec-table");
      //console.log('data', rows);
      // 這裡你可以把 defs.table_columns / defs.filter_item_coldict 傳給 table JS 去初始化
      document.dispatchEvent(new CustomEvent("aoi_density:subtab-table", {
        detail: { tabKey, config: defs, data: rows }
      }));

    } else if (type === "Chart") {
      // === type: "Chart" → Trend Chart 頁 ===
      AOI.showSection("aoi_density-Chart");

      // 這裡把 config 丟給 chart 區的 JS 去畫圖
      document.dispatchEvent(new CustomEvent("aoi_density:subtab-chart", {
  ...
  AOI.buildRightSubTabs = function(containerEl){
    ensureAfterDataReady(()=>{
      ...
      keys.forEach(k=>{
        ...
        const label = conf.tab_name || k;              // 顯示名稱：有 tab_name 用 tab_name，沒有就用 key
        ...
        btn.addEventListener("click", ()=>{
          ...
          // ★ 呼叫新版本的 applySubTab，讓它決定要顯示哪個 section
          AOI.applySubTab(k);
        });
      ...
      if (!AOI.state.activeSubTab && keys.length) {
        ...
        // 若已經有 activeSubTab（例如重新切回 Density），也同步 section 顯示
