...
class MySQLConnetFunc:
    ...
    def get_table_by_days(self, table_name, cols=None, day=None, time_key=None):
        """
        ...
        try:
            ...
            logging.info(f"[get_table_by_days] 讀取完成：{len(df)} rows.")
            return df

        except SQLAlchemyError as e:
            logging.error(f"[get_table_by_days] 讀取 '{table_name}' 發生錯誤: {e}")
            return pd.DataFrame()
"""
