...
(function () {
  ...
  AOI.getFiltered = function(opts){
    ...
    // ===== 尺寸投影：把未勾選的尺寸歸零，並重算 defect_num / density =====
    const selectedSizes = Array.isArray(filters?.[sizeKey]) ? new Set(filters[sizeKey]) : null;
    if (selectedSizes && selectedSizes.size > 0) {
      out = out.map(r => {
        // 原始值（不要就地覆蓋 AOI.state.rows，這裡回傳新物件）
        const nG = Number(r.n_glasses ?? r.maingroup_glass_count ?? r.total_glass_count ?? 0);

        // 依當前選擇保留 / 歸零
        const s = selectedSizes.has('S') ? Number(r.s_count ?? r.small_defect_count ?? 0) : 0;
        const m = selectedSizes.has('M') ? Number(r.m_count ?? r.middle_defect_count ?? 0) : 0;
        const l = selectedSizes.has('L') ? Number(r.l_count ?? r.large_defect_count  ?? 0) : 0;
        const o = selectedSizes.has('O') ? Number(r.o_count ?? r.over_defect_count   ?? 0) : 0;
        ...
        return {
          ...
          // Chart 友善欄位（舊欄位相容）
          s_count: s, m_count: m, l_count: l, o_count: o,
