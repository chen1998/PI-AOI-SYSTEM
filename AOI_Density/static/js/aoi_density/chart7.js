...
(function () {
  ...
  function buildColumnsByAoiCode(rows, globalOrder, specIndex, selectedSizesArr){
    ...
    function calcSelectedBySize(bucketRows){
      const perGlass = new Map();  // glass_id -> {S,M,L,O}
      let totalS = 0, totalM = 0, totalL = 0, totalO = 0;

      bucketRows.forEach(rec => {
        // 這裡的 s/m/l/o 已經是 service.getFiltered 投影後的值
        ...
        if (gdc && typeof gdc === "object" && !Array.isArray(gdc)) {
          Object.entries(gdc).forEach(([gid, stat]) => {
            const st = parseSizeStats(stat);   // 支援 "S:1..." 或 {S:..}
      ...
      if (perGlass.size > 0) {
        ...
        // ★ 沒有逐片尺寸資訊：退回用 service.js 投影後的 defect_code_glass_count
        //   （避免 perGlass.size>0 但都 0 的情況卡死）
        bucketRows.forEach(rec => {
          const v = Number(
          ...
          if (v > glassSel) glassSel = v;   // 多行情況取 max，比直接相加安全
        });
      }
